...
def get_choice(
  *,
  ...
  if not crud.quiz_question.has_choice(db, question_id=question.id, choice_id=choice.id):
    raise HTTPException(status_code=404, detail="Question does not belong to the specified quiz")
...
def get_attempt_and_link(
  *,
  ...
  link = crud.quiz_student_link.get_by_quiz_and_student_ids(db, quiz_id=quiz.id, student_id=current_student.id)
  if not link:
...
def get_current_author(
  *,
  ...
  if not (
    crud.user.is_superuser(current_user) or 
    crud.quiz.is_author(db, user_id=current_user.id, quiz_index=quiz.id)
  ):
    raise HTTPException(
      status_code=400, detail="The user doesn't have enough privileges"
    )
