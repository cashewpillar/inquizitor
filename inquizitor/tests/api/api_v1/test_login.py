...
async def test_get_tokens(client: AsyncClient) -> None:
    r = await client.post("/login/token", data=LOGIN_DATA)
...
async def test_use_access_token(
    ...
    r = await client.get("/users/profile", cookies=await superuser_cookies)
    result = r.json()
    assert r.status_code == 200
...
async def test_use_refresh_token(
    ...
    r = await client.post("/login/refresh", cookies=await superuser_cookies)
    result = r.json()
    assert r.status_code == 200
    assert "access_token_cookie" in r.cookies
    assert r.cookies["access_token_cookie"]
...
async def test_access_revoke(
    ...
    r = await client.delete("/login/access-revoke", cookies=cookies)
    assert r.status_code == 200
    assert "msg" in r.json()
    assert "Access" in r.json()["msg"]

    r = await client.get("/users/profile", cookies=cookies)
    result = r.json()
